...
const track = {
    gate: "00-09992",
    loops: [
        { name: "Hill", min: 2000, max: 3000 },
        { name: "Finish", min: 30000, max: 70000 }
    ]
};
test('Hill crossing after 2000 and before 3000 should result in Hill Time', () => {
    ...
    expect(result.passings.length).toBe(1);
    expect(result.passings[0]).toBe(2000);
});
...
test('Finish crossing after 30000 should result in time when rider has valid hill start time', () => {
    ...
    expect(result.gate).toBe(gate);
    expect(result.passings.length).toBe(2);
    expect(result.passings[0]).toBe(3000);
    expect(result.passings[1]).toBe(30000);
});
